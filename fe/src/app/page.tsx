...
export default function Home() {
  ...
  const getTagColor = (tag: string) => {
    const colors: any = {
      'Freshman': 'bg-slate-500', 'Sophomore': 'bg-emerald-500', 'Junior': 'bg-amber-500', 
      'Senior': 'bg-orange-500', 'Graduate': 'bg-violet-500', 'PhD': 'bg-rose-500', 'Alumni': 'bg-red-500',
      'CS Major': 'bg-indigo-500', 'Business Major': 'bg-teal-500', 'Engineering Major': 'bg-cyan-500',
      'Meta': 'bg-blue-500', 'Google': 'bg-red-500', 'Amazon': 'bg-orange-500', 
      'Apple': 'bg-gray-500', 'Microsoft': 'bg-sky-500', 'Netflix': 'bg-red-500',
      'Python': 'bg-blue-500', 'Java': 'bg-red-500', 'JavaScript': 'bg-yellow-500', 'React': 'bg-cyan-500',
      'Machine Learning': 'bg-purple-500', 'Data Science': 'bg-green-500', 'Web Dev': 'bg-indigo-500',
      'Mobile Dev': 'bg-teal-500', 'Cloud Computing': 'bg-sky-500', 'Cybersecurity': 'bg-red-500',
      'Leadership': 'bg-amber-500', 'Teamwork': 'bg-lime-500', 'Public Speaking': 'bg-rose-500',
      'Research': 'bg-violet-500', 'Hackathons': 'bg-fuchsia-500', 'Open Source': 'bg-emerald-500',
      'Startups': 'bg-orange-500', 'Internship': 'bg-blue-500'
    };
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-900 text-center mt-4">Loading...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white flex">
      <div className="w-64 bg-white border-r border-gray-200 p-6">
        ...
        <nav className="space-y-4">
          <button onClick={() => { setActiveSection('home'); localStorage.setItem('activeSection', 'home'); }} className="w-full flex items-center gap-3 p-3 rounded-lg text-left" style={{backgroundColor: activeSection === 'home' ? 'rgba(166, 25, 46, 0.2)' : 'transparent'}}>
            ...
            <span className="font-medium text-gray-900">AskSDSU</span>
          </button>
          ...
          <button onClick={() => { setActiveSection('courses'); localStorage.setItem('activeSection', 'courses'); }} className="w-full flex items-center gap-3 p-3 rounded-lg text-left" style={{backgroundColor: activeSection === 'courses' ? 'rgba(166, 25, 46, 0.2)' : 'transparent'}}>
            ...
            <span className="font-medium text-gray-900">Course Compass</span>
          </button>
          ...
          <button onClick={() => { setActiveSection('peer'); localStorage.setItem('activeSection', 'peer'); }} className="w-full flex items-center gap-3 p-3 rounded-lg text-left" style={{backgroundColor: activeSection === 'peer' ? 'rgba(166, 25, 46, 0.2)' : 'transparent'}}>
            ...
            <span className="font-medium text-gray-900">Peer Connect</span>
          </button>
          ...
          <button onClick={() => { setActiveSection('clubs'); localStorage.setItem('activeSection', 'clubs'); }} className="w-full flex items-center gap-3 p-3 rounded-lg text-left" style={{backgroundColor: activeSection === 'clubs' ? 'rgba(166, 25, 46, 0.2)' : 'transparent'}}>
            ...
            <span className="font-medium text-gray-900">Clubs</span>
          </button>
      ...
      <div className="flex-1 p-6 overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-lg p-6">
              <textarea
                ...
                className="w-full px-4 py-3 bg-gray-50 text-gray-900 rounded-lg border border-gray-300 focus:outline-none mb-3"
                rows={3}
                value={newPost}
              ...
              <div className="flex gap-3">
                <select
                  ...
                  className="px-4 py-2 bg-gray-50 text-gray-900 rounded-lg border border-gray-300 focus:outline-none"
                >
                  {categories.map(cat => (
            ...
            <div className="mb-4">
              <select
                ...
                className="px-4 py-2 bg-white text-gray-900 rounded-lg border border-gray-300 focus:outline-none"
              >
              ...
              <div key={post.id} className="bg-white rounded-lg shadow p-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-900 font-medium">{post.likes_count}</span>
                    <button
                      onClick={() => handleVote(post.id, -1)}
                      className="p-1 rounded transition"
                      style={{color: post.user_vote === -1 ? '#A6192E' : '#9CA3AF'}}
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    <div className="flex gap-2 mb-4">
                      <input
                        ...
                        className="flex-1 px-4 py-2 bg-gray-50 text-gray-900 rounded-lg border border-gray-300 focus:outline-none text-sm"
            ...
            <div className="mb-6">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 bg-white text-gray-900 rounded-lg border border-gray-300 focus:outline-none"
                  value={searchQuery}
              ...
              {searchResults.length > 0 && (
                <div className="grid grid-cols-4 gap-4 mt-4">
                  {searchResults.map(result => (
                    <div key={result.id} className="bg-white rounded-lg overflow-hidden cursor-pointer hover:bg-gray-50 transition border border-gray-200" onClick={() => setSelectedUser(result)}>
                      ...
                      <div className="p-4">
                        <p className="text-gray-900 font-semibold text-lg mb-3">{result.username}</p>
                        {result.bio && <p className="text-gray-700 text-xs mb-3 line-clamp-2">{result.bio}</p>}
                        {result.tags && result.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mb-3">
                            {result.tags.slice(0, 3).map((tag: string) => (
                              <span key={tag} className={`px-2 py-1 text-white text-xs rounded ${getTagColor(tag)}`}>{tag}</span>
                            ))}
                            {result.tags.length > 3 && <span className="px-2 py-1 bg-gray-100 text-gray-900 text-xs rounded">+{result.tags.length - 3}</span>}
                          </div>
                        )}
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-4">
                  {connections.filter(c => c.status === 'pending' && !c.is_sender).map(conn => (
                    <div key={conn.id} className="bg-white rounded-lg overflow-hidden border border-gray-200">
                      ...
                      <div className="p-4">
                        <p className="text-gray-900 font-semibold text-lg mb-3">{conn.username}</p>
                        {conn.bio && <p className="text-gray-700 text-xs mb-3 line-clamp-2">{conn.bio}</p>}
                        {conn.tags && conn.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mb-3">
                            {conn.tags.slice(0, 3).map((tag: string) => (
                              <span key={tag} className={`px-2 py-1 text-white text-xs rounded ${getTagColor(tag)}`}>{tag}</span>
                            ))}
                            {conn.tags.length > 3 && <span className="px-2 py-1 bg-gray-100 text-gray-900 text-xs rounded">+{conn.tags.length - 3}</span>}
                          </div>
                        )}
                        <button
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-4">
                  {connections.filter(c => c.status === 'accepted').map(conn => (
                    <div key={conn.id} className="bg-white rounded-lg overflow-hidden cursor-pointer hover:bg-gray-50 transition border border-gray-200" onClick={() => setSelectedUser(conn)}>
                      <img src={`https://ui-avatars.com/api/?name=${conn.username}&size=300&background=random&color=fff&bold=true`} alt={conn.username} className="w-full h-32 object-cover" />
                      <div className="p-4">
                        <p className="text-gray-900 font-semibold text-lg mb-3">{conn.username}</p>
                        {conn.bio && <p className="text-gray-700 text-xs mb-3 line-clamp-2">{conn.bio}</p>}
        ...
        {activeSection === 'courses' && (
          <div>
            {selectedCourse ? (
              <div>
                <button onClick={() => { setSelectedCourse(null); setSelectedSection(null); }} className="text-gray-600 hover:text-gray-900 mb-4">← Back to Courses</button>
                <div className="bg-white rounded-lg p-5 border border-gray-200 mb-4">
                  ...
                  <p className="text-lg text-gray-700 mt-1">{selectedCourse.title}</p>
                  <p className="text-gray-600 mt-2">{selectedCourse.description}</p>
                </div>
                <div className="bg-white rounded-lg p-5 border border-gray-200 mb-4">
                  <h3 className="font-bold text-lg mb-3 text-gray-900">Select Section</h3>
                  <div className="space-y-2">
                    {courseSections.map((section: any) => (
                      ...
                      }} className={`p-4 rounded-lg cursor-pointer ${selectedSection?.section_id === section.section_id ? 'bg-gray-50 border-2' : 'border hover:bg-gray-50'}`} style={selectedSection?.section_id === section.section_id ? {borderColor: '#A6192E'} : {borderColor: '#D1D5DB'}}>
                        <div className="flex justify-between">
                          <div>
                            <div className="font-semibold text-gray-900">Section {section.section_number} - {section.professor_name}</div>
                            <div className="text-sm text-gray-600">{section.schedule} • {section.location}</div>
                            {section.tags && <div className="flex gap-2 mt-2">{section.tags.split(',').map((tag: string, i: number) => {
                              const colors = ['bg-slate-500 text-white', 'bg-orange-500 text-white', 'bg-rose-500 text-white', 'bg-teal-500 text-white', 'bg-amber-500 text-white', 'bg-indigo-500 text-white'];
                          ...
                          <div className="text-2xl font-bold text-gray-900">{section.professor_rating.toFixed(1)}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {selectedSection && (
                  <div key={selectedSection.section_id} className="bg-white rounded-lg border border-gray-200">
                    <div className="flex gap-4 border-b border-gray-200 px-5 pt-3">
                      <button onClick={() => setReviewTab('reviews')} className={`pb-3 px-2 font-medium ${reviewTab === 'reviews' ? 'border-b-2 text-gray-900' : 'text-gray-500'}`} style={reviewTab === 'reviews' ? {borderColor: '#A6192E'} : {}}>Reviews ({sectionReviews.length})</button>
                      <button onClick={() => setReviewTab('qa')} className={`pb-3 px-2 font-medium ${reviewTab === 'qa' ? 'border-b-2 text-gray-900' : 'text-gray-500'}`} style={reviewTab === 'qa' ? {borderColor: '#A6192E'} : {}}>Q&A ({sectionQuestions.length})</button>
                    </div>
                    <div className="p-5 space-y-4">
                      {reviewTab === 'reviews' ? (
                        sectionReviews.length > 0 ? sectionReviews.map((r: any) => (
                          <div key={r.id} className="border-b border-gray-200 pb-4">
                            ...
                            {r.tags && <div className="flex gap-2 mt-2">{r.tags.split(',').map((tag: string, i: number) => {
                              const colors = ['bg-slate-500 text-white', 'bg-emerald-500 text-white', 'bg-violet-500 text-white', 'bg-orange-500 text-white', 'bg-rose-500 text-white', 'bg-teal-500 text-white'];
                              return <span key={i} className={`text-xs px-2 py-1 rounded ${colors[i % colors.length]}`}>{tag}</span>;
                            })}</div>}
                          </div>
                        )) : <div className="text-center text-gray-600 py-8">No reviews yet for this section</div>
                      ) : (
                        ...
                        )) : <div className="text-center text-gray-600 py-8">No questions yet for this section</div>
                      )}
                    </div>
                  </div>
                )}
              ...
              <div>
                <button onClick={() => { setSelectedProfessor(null); setProfCourses([]); }} className="text-gray-600 hover:text-gray-900 mb-4">← Back to Professors</button>
                <div className="bg-white rounded-lg p-5 border border-gray-200 mb-4">
                  <div className="flex justify-between">
                    <div>
                      <h2 className="text-2xl font-bold text-gray-900">{selectedProfessor.name}</h2>
                      <p className="text-gray-600">{selectedProfessor.department}</p>
                    </div>
                    {selectedProfessor.total_reviews > 0 && <div className="text-right"><div className="text-4xl font-bold" style={{color: '#A6192E'}}>{selectedProfessor.avg_rating.toFixed(1)}</div><div className="text-sm text-gray-600">{selectedProfessor.total_reviews} reviews</div></div>}
                  </div>
                  {selectedProfessor.total_reviews > 0 && (
                    <div className="grid grid-cols-3 gap-4 mt-4 text-sm">
                      <div><span className="text-gray-600">Difficulty: </span><span className="font-bold text-gray-900">{selectedProfessor.avg_difficulty.toFixed(1)}/5.0</span></div>
                      <div><span className="text-gray-600">Take Again: </span><span className="font-bold" style={{color: '#A6192E'}}>{selectedProfessor.would_take_again_percent.toFixed(0)}%</span></div>
                    </div>
                  )}
                </div>
                <div className="bg-white rounded-lg p-5 border border-gray-200">
                  <h3 className="font-bold text-lg mb-3 text-gray-900">Courses Taught</h3>
                  <div className="space-y-3">
                    {profCourses.map((c: any) => (
                      <div key={c.section_id} onClick={async () => {
                        ...
                        <h4 className="font-semibold text-gray-900">{c.course_code} - {c.course_title}</h4>
                        <div className="text-sm text-gray-600">Section {c.section_number} • {c.semester}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <>
                <div className="flex gap-3 mb-4">
                  <input type="text" placeholder={courseTab === 'courses' ? 'Search courses (e.g., CS-160)' : 'Search professors'} value={courseSearchQuery} onChange={(e) => setCourseSearchQuery(e.target.value)} className="flex-1 px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-lg focus:outline-none" />
                  <select value={selectedDept} onChange={(e) => setSelectedDept(e.target.value)} className="px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-lg focus:outline-none">
                    <option value="">All Departments</option>
                    {departments.map((dept) => <option key={dept} value={dept}>{dept}</option>)}
                  </select>
                </div>
                <div className="flex gap-4 border-b border-gray-200 mb-4">
                  <button onClick={() => setCourseTab('courses')} className={`pb-3 px-2 font-medium ${courseTab === 'courses' ? 'border-b-2 text-gray-900' : 'text-gray-500'}`} style={courseTab === 'courses' ? {borderColor: '#A6192E'} : {}}>Courses</button>
                  <button onClick={() => setCourseTab('professors')} className={`pb-3 px-2 font-medium ${courseTab === 'professors' ? 'border-b-2 text-gray-900' : 'text-gray-500'}`} style={courseTab === 'professors' ? {borderColor: '#A6192E'} : {}}>Professors</button>
                </div>
                {courseTab === 'courses' ? (
              <div className="space-y-3">
                {courses.length === 0 ? <div className="bg-white rounded-lg p-8 text-center text-gray-600">Loading courses...</div> : courses.map((course: any) => (
                  ...
                  }} className="bg-white rounded-lg p-5 hover:bg-gray-50 transition cursor-pointer border border-gray-200">
              ...
              <div className="space-y-3">
                {courseProfessors.length === 0 ? <div className="bg-white rounded-lg p-8 text-center text-gray-600">No professors found.</div> : courseProfessors.map((prof: any) => (
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        {prof.total_reviews > 0 && <div className="flex gap-4 mt-3 text-sm">
                          ...
                          <div><span className="text-gray-500">Difficulty: </span><span className="font-semibold text-gray-900">{prof.avg_difficulty.toFixed(1)}/5.0</span></div>
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-gray-900">{prof.avg_rating.toFixed(1)}</div>
                        <div className="text-xs text-gray-600">{prof.total_reviews} reviews</div>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {messages.map(msg => (
              <div key={msg.id} className="bg-white rounded-lg shadow p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900">{msg.is_sender ? msg.receiver_username : msg.sender_username}</p>
        ...
        {activeSection === 'profile' && (
          <div className="max-w-2xl">
            <div className="bg-white rounded-lg p-6 space-y-4 border border-gray-200">
                ...
                {!isEditing ? (
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-4 py-2 bg-gray-100 text-white rounded-lg text-sm"
                    >
                      Cancel
                    </button>
                    <button
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {isEditing ? (
                    <input
                      type="text"
                      className="w-full px-4 py-2 bg-gray-50 text-gray-900 rounded-lg border border-gray-300 focus:outline-none"
                      value={profileData.username}
                      onChange={(e) => setProfileData({...profileData, username: e.target.value})}
                    />
                  ) : (
                    <p className="text-gray-900 px-4 py-2">{user.username}</p>
                  )}
                </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-3">
                  <label className="block text-sm font-medium text-gray-600">My Tags</label>
                  {!showTagSelector && (
                    <button
                ...
                {showTagSelector && (
                  <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-300">
                    <div className="flex justify-between items-center mb-3">
                      <p className="text-gray-900 text-sm font-medium">Select a tag to add:</p>
                      <button
                    ...
                    userTags.map(tag => (
                      <div key={tag.id} className={`px-3 py-2 text-white text-sm rounded ${getTagColor(tag.name)} flex items-center gap-2`}>
                        ...
                        <button 
                          ...
                          className="hover:text-red-200 font-bold text-lg leading-none"
                          title="Remove tag"
                        >
                          ×
                        </button>
            ...
            <div className="bg-white rounded-lg p-6 space-y-4 mt-6 border border-gray-200">
                ...
                <div className="space-y-3">
                  {userEvents.filter(event => event.action !== 'Join').map((event, index) => (
                    <div key={index} className="bg-gray-50 rounded-lg p-4">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-gray-900 font-medium">{event.eventName}</p>
                          <p className="text-gray-600 text-sm">{event.action} on {event.date}</p>
                        </div>
      ...
      <div className="w-80 bg-white border-l border-gray-200 p-6">
        <h2 className="text-xl font-bold mb-6 text-gray-900">{activeSection === 'courses' ? 'Course Info' : 'Trending'}</h2>
        <div className="space-y-6">
          {activeSection === 'courses' ? (
            <>
              <div>
                <h3 className="text-sm font-semibold text-gray-600 mb-3">🔥 Popular Professors</h3>
                <div className="space-y-3">
                  {popularProfs.length > 0 ? popularProfs.slice(0, 5).map((prof: any) => (
                    <div key={prof.id} onClick={async () => {
                      ...
                      <div className="font-medium text-sm text-gray-900">{prof.name}</div>
                      <div className="text-xs text-gray-600">{prof.department}</div>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs font-semibold" style={{color: '#A6192E'}}>{prof.avg_rating.toFixed(1)}</span>
                        <span className="text-xs text-gray-500">•</span>
                        <span className="text-xs text-gray-600">{prof.total_reviews} reviews</span>
                      </div>
                    </div>
                  )) : <p className="text-sm text-gray-600">No professors yet</p>}
                </div>
              </div>
            ...
            <>
              <div>
                <h3 className="text-sm font-semibold text-gray-600 mb-3">Topics</h3>
            <div className="space-y-2">
              <div className="p-3 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer">
                <p className="text-gray-900 font-medium">#SDSULife</p>
                <p className="text-xs text-gray-600">2.3k posts</p>
              </div>
              <div className="p-3 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer">
                <p className="text-gray-900 font-medium">#StudyGroup</p>
                <p className="text-xs text-gray-600">1.8k posts</p>
              </div>
              <div className="p-3 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer">
                <p className="text-gray-900 font-medium">#AztecPride</p>
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-600 mb-3">Top Mentors</h3>
            <div className="space-y-2">
              <div className="p-3 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer">
                <p className="text-gray-900 font-medium">Dr. Sarah Chen</p>
                <p className="text-xs text-gray-600">Computer Science</p>
              </div>
              <div className="p-3 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer">
                <p className="text-gray-900 font-medium">Prof. Mike Johnson</p>
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-600 mb-3">Hot Posts</h3>
            <div className="space-y-2">
              <div className="p-3 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer">
                <p className="text-gray-900 text-sm">Spring Break Plans 🌴</p>
                <p className="text-xs text-gray-600">156 likes</p>
              </div>
              <div className="p-3 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer">
                <p className="text-gray-900 text-sm">Career Fair Tips</p>
      ...
      {selectedUser && (
        <div className="fixed inset-0 bg-white bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setSelectedUser(null)}>
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-gray-200" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  {selectedUser.clubs && (
                    <div className="bg-gray-50 rounded-lg p-4 mb-4 border border-gray-200">
                      <h3 className="text-sm font-semibold text-gray-600 mb-2">🎯 Clubs</h3>
                      <div className="flex flex-wrap gap-2">
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 mb-4 border border-gray-200">
                    <div className="mb-3 flex items-center justify-between">
                      <h4 className="text-gray-900 font-medium">{calendarDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</h4>
                      <div className="flex gap-2">
                        <button onClick={(e) => { e.stopPropagation(); setCalendarDate(new Date(calendarDate.getFullYear(), calendarDate.getMonth() - 1)); }} className="text-gray-600 hover:text-gray-900 transition">
                        ...
                        for (let d = 1; d <= daysInMonth; d++) {
                          ...
                          days.push(
                            <div key={d} className={`text-sm py-3 rounded-lg font-medium transition-all cursor-pointer ${
                              isAvailable
                                ? 'bg-gradient-to-br from-green-500 to-green-600 text-gray-900 shadow-lg hover:shadow-xl hover:scale-105' 
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-3">Peer Reviews ({selectedUser.id % 3 + 2})</h3>
                  <div className="space-y-3">
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-gray-600 text-sm">by @student{selectedUser.id + 1}</span>
                      </div>
                      <p className="text-gray-700 text-sm">Great study partner! Very knowledgeable and patient. Helped me ace my exam.</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="flex">
                          {[1,2,3,4].map(s => <span key={s} className="text-yellow-400">★</span>)}<span className="text-gray-500">★</span>
                        </div>
                        <span className="text-gray-600 text-sm">by @student{selectedUser.id + 2}</span>
                      </div>
                      <p className="text-gray-700 text-sm">Really helpful with project work. Knows the material well and explains clearly.</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="flex">
                        ...
                        <span className="text-gray-600 text-sm">by @student{selectedUser.id + 3}</span>
                      </div>
                      <p className="text-gray-700 text-sm">Awesome mentor! Made complex topics easy to understand. Highly recommend!</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {eventModal.isOpen && (
        <div className="fixed inset-0 bg-white bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-gray-900 mb-4">{eventModal.action} for Event</h3>
            <p className="text-gray-700 mb-6">Are you sure you want to {eventModal.action.toLowerCase()} for "{eventModal.eventName}"?</p>
      ...
      {showSyllabus && (
        <div className="fixed inset-0 bg-white bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowSyllabus(false)}>
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-gray-200" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <h2 className="text-2xl font-bold text-gray-900">Course Syllabus</h2>
                <button onClick={() => setShowSyllabus(false)} className="text-gray-600 hover:text-gray-900">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
                </button>
              </div>
              <pre className="text-gray-700 whitespace-pre-wrap font-mono text-sm">{syllabusContent}</pre>
            </div>
          </div>
        </div>
      )}
